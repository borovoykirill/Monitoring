...
# Install  and configure Zabbix java gateway
sudo yum -y install zabbix-java-gateway
sudo systemctl enable zabbix-java-gateway
sudo systemctl start zabbix-java-gateway

sudo sed -i 's@# JavaGateway=@JavaGateway=10.10.1.11@' /etc/zabbix/zabbix_server.conf
